...
### Domain, solver and portable contract products

`cargo xtask generate` owns the domain, solver and portable contract products below. `cargo xtask generate-check` verifies their complete inventory and exact bytes.

| Authoritative input | Checked-in outputs |
|---|---|
| `schemas/domain-packs/official-test.contract.json` (source schema version 3) | `crates/eutheto-command/src/generated_official_test_pack_contract.rs`, shared `apps/desktop/src/api/generated-domain-pack-contracts.ts`, `schemas/generated/official-test.command-schemas.json`, `schemas/generated/official-test.internal.schema.json`, `schemas/generated/official-test.portable.schema.json`, `schemas/generated/official-test.share-result.schema.json`, `xtask/generated/official-test-ai-tools.json`, `xtask/generated/official-test-ui-manifest.json`, and `docs/generated/official-test-pack-contract.md` |
| `schemas/domain-packs/workforce.contract.json` (source schema version 3) | `domains/workforce/core/src/generated_workforce_pack_contract.rs`, shared `apps/desktop/src/api/generated-domain-pack-contracts.ts`, `schemas/generated/workforce.command-schemas.json`, `schemas/generated/workforce.internal.schema.json`, `schemas/generated/workforce.portable.schema.json`, `xtask/generated/workforce-ai-tools.json`, `xtask/generated/workforce-ui-manifest.json`, and `docs/generated/workforce-pack-contract.md`; no Workforce Share Result product or production registration |
...
| Immutable `tests/migration/fixtures/portable_v1_scenario.json`, the synthetic pack contract, and current Rust portable-envelope serialization | `tests/migration/fixtures/portable_v2_scenario.json`; generated by `xtask/src/phase02_generate.rs` and checked against the real current export in permanent migration tests |

The generator parses both pack sources and the solver matrix with unknown-field rejection, validates sorted unique identities and stage-appropriate metadata, and emits stable JSON or deterministic source/document tables. Pack-source v3 permits only bounded, acyclic `#/$defs/Name` references in schema positions; definitions are expanded and removed before runtime output. The synthetic pack retains complete metadata and Share Result; Workforce's unimplemented score/provenance/result/transfer groups remain empty. Generated source and Markdown headers bind exact authoritative bytes by BLAKE3; AI/UI metadata wrappers also carry the source digest. Standalone runtime schemas do not receive unsupported ownership fields.

The combined TypeScript product contains both packs' command IDs and structural payload types. Generation invokes installed Prettier 3.9.6 with the checked-in desktop configuration, rejects an ignored product, and records formatter/config provenance. Run `just install` first; generation never installs dependencies. Both generation and drift checking use these formatted bytes.

Pack inventory ownership is prefix-scoped: `official-test.`/`workforce.` under `schemas/generated`, `official-test-`/`workforce-` under `xtask/generated`, the two pack-document prefixes under `docs/generated`, and the declared generated Rust/TypeScript source prefixes. Drift checking rejects stale matching products; generation removes them without owning unrelated shared-directory files. This includes rejecting/removing a Workforce Share Result file while that product is absent.

The source-format version is independent of persisted document and portable versions. `DomainCatalog.internalSchema` describes only the four-map `ScenarioDomain`, under `scenarioSchemaVersion`; it does not add a second internal version field or host envelope. Field-level string/array bounds use the shared schema evaluator, while portable byte/node ceilings and typed identity/reference validation remain separate requirements.
