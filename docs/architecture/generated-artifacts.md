...
| Tauri ACL schemas/manifests under `apps/desktop/src-tauri/gen/schemas/` and generated plugin permissions under `apps/desktop/src-tauri/permissions/autogenerated/` | Tauri configuration, capabilities, permissions, command/plugin metadata, and the pinned Tauri dependency/CLI set | The Tauri CLI/build invoked by the native development/build recipes; ignored local build output, not `xtask` products | Native desktop shell |
...
- `apps/desktop/src-tauri/gen/schemas/` and `apps/desktop/src-tauri/permissions/autogenerated/` contain Tauri-owned build output and are ignored by Git. The pinned Tauri CLI/build emits these bytes from the checked-in native inputs. They are not `xtask` products and must not be hand-edited or rewritten by the general frontend formatter.

Native build verification exercises generation from the pinned Tauri CLI and authoritative configuration, capability, permission, command/plugin metadata, and dependency inputs. Commit changes to those inputs, not the ignored generated schemas or permissions. Project-owned generated DTOs remain checked-in drift-checked artifacts; the native build-output exception does not apply to them.
