...
The fourth debug run's 100-person × 64-type membership preview/apply took 54,774/25,990 ms; its 200-shift stress fixture admission took 60,357 ms. In the recorded performance artifact, the large-supported Work initial/full-window action-to-double-animation-frame proxies were 368/190 ms, while 100 × 64 eligibility cold input/scroll proxies were 10/26 ms and warm proxies 6/12 ms. These are different operation boundaries on one unaccelerated Linux debug WebKit run, not INP, a release-build service-level guarantee, or proof that large-batch edits feel fast.
