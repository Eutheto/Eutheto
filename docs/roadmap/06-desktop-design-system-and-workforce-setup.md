...
The first hosted [PR58](https://github.com/Eutheto/Eutheto/pull/58) run selected the full native and packaged-worker matrices. The Linux x86_64 and arm64 native jobs completed WebDriver acceptance but were cancelled during the final desktop launch smoke at their exact 75- and 90-minute job limits; the native policy correctly rejected the incomplete matrix. The Portable workflow now allows 120 minutes for those two Linux jobs without deferring targets or weakening the gate. A fresh passing exact-head hosted run is still required.
