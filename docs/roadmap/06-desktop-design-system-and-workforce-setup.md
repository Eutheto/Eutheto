...
The first hosted [PR58](https://github.com/Eutheto/Eutheto/pull/58) run selected the full native and packaged-worker matrices. Linux x86_64 and arm64 completed WebDriver acceptance but were cancelled during the final desktop launch smoke at their exact 75- and 90-minute job limits; native policy correctly rejected the incomplete matrix. The corrected head `0979235d2a4f8b3cfe24f6d0f62b025354f5001d` allowed 120 minutes for those two jobs, then [passed](https://github.com/Eutheto/Eutheto/actions/runs/36224414554) all six native source targets and the native policy. Its [source](https://github.com/Eutheto/Eutheto/actions/runs/36224414557), [packaged-worker](https://github.com/Eutheto/Eutheto/actions/runs/36224414616), [security](https://github.com/Eutheto/Eutheto/actions/runs/36224414570), and [immutable-input](https://github.com/Eutheto/Eutheto/actions/runs/36224414560) workflows also passed; the optional OR-Tools candidate source build was skipped, not tested.

### Exact integrated package-10 checkpoint — 26 September 2026

[PR58](https://github.com/Eutheto/Eutheto/pull/58) was squash-merged into protected `phase/06-desktop` as `641fa203a1077cd7875c31f8f2f933d15a14e012`. Its tree matched the reviewed source, and the trusted initial-phase-base DCO checker passed for all seven commits through the squash. The five required manual-dispatch workflows all completed successfully on **that exact integrated SHA**: [PR/source](https://github.com/Eutheto/Eutheto/actions/runs/36229086075), [Portable](https://github.com/Eutheto/Eutheto/actions/runs/36229085517), [OR-Tools Worker](https://github.com/Eutheto/Eutheto/actions/runs/36229085474), [Security](https://github.com/Eutheto/Eutheto/actions/runs/36229085016), and [Dependency policy](https://github.com/Eutheto/Eutheto/actions/runs/36229085383). Portable ran all six required native source targets; the worker ran all four approved package targets. Portable's optional candidate-source job was explicitly not run.

**Exit reconciliation:** The implemented setup, reviewed native mutations, CSV/portable/backup flows, validation navigation, keyboard focus, status and measured responsiveness have the local and hosted automated evidence above plus the maintainer's applicable-step manual report. A focused calculation of 19 semantic foreground/background pairs from the light/dark CSS tokens found minimum normal-text contrast ratios of 6.17:1 (light) and 4.75:1 (dark); selected Chromium axe checks and the maintainer's light/dark native zoom inspection supplement that palette check. No numerical, route-wide rendered contrast audit, spoken screen-reader observation, independent solver-naive comprehension study or other-platform manual shortcut check was performed. Spoken screen-reader and solver-naive activities are deferred to beta under the dated exception, not passed; the additional rendered/platform observations remain unverified rather than implied by the token and hosted-build evidence. Protected-`main` integration is still required before Phase 07 production; none of these checkpoint workflows is an installer or release-candidate test.
