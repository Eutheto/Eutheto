...
The unregistered `eutheto_workforce::temporal` APIs (`resolve_shifts`, `preview_generation`, `resolve_calendar`, and `reporting_window`) return bounded operation-local Rust values. They do not register the pack, persist edits or approvals, establish eligibility/feasibility, or act as a verifier. Production command routing and durable review/apply remain dependency-gated on the completed pack.
...
The unregistered `eutheto_workforce::assignment_rules` APIs (`analyze_assignments`, `compile_assignment_rules`, and `evaluate_assignment_rules`) contribute bounded graph/readiness data, solver-neutral mathematics and independent original-domain rule summaries. They do not construct a complete pack, project/accept a result, score, persist or route Workforce application commands. Staged registration and all rule-completion gates below remain unchanged.
