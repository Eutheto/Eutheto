...
class OperationScope<C extends OperationContextV1> {
  ...
  run<T>(
    ...
    const libraryPurpose = purpose.kind === "settingsImportPreview" ||
    ...
    if (this.#context.kind === "library" &&
      ((purpose.kind === "settingsImportApply") !== (this.#context.expectedLibraryRevision !== null) ||
        command !== (purpose.kind === "settingsExport" ? "settings_export_nonsecret" : "settings_import_nonsecret")))
      throw new RangeError("The settings operation requires its matching command and revision context");
