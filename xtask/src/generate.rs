...
export interface ApplicationSettingValues {
  readonly appearance: { readonly theme?: "system" | "light" | "dark"; readonly reducedMotion?: boolean };
  readonly locale: string;
  readonly units: "metric" | "us-customary";
}
export type ApplicationSettingKey = keyof ApplicationSettingValues;
export interface ApplicationSettingEntryV1<K extends ApplicationSettingKey = ApplicationSettingKey> {
...
export type SettingsImportResultV1 = SettingsImportPreviewV1 | SettingsImportAppliedV1 | SettingsImportDiscardedV1;
...
export type SettingsImportRequestV1 =
  | { readonly action: "preview"; readonly schemaVersion: 1; readonly requestId: UuidV7; readonly operationId: UuidV7 }
  | {
      readonly action: "apply"; readonly schemaVersion: 1; readonly requestId: UuidV7;
      readonly operationId: UuidV7; readonly previewId: UuidV7; readonly approvalSha256: string;
      readonly expectedLibraryRevision: Revision;
    }
  | { readonly action: "discard"; readonly schemaVersion: 1; readonly requestId: UuidV7; readonly target: SettingsPreviewTarget };
...
export interface LicenseInventoryV2 {
  ...
  readonly authoritativeInputs: readonly ["Cargo.lock", "pnpm-lock.yaml", "xtask/supply-chain-inputs.json"];
...
const isPortableApplied = shape<PortableAppliedDto>({
  schemaVersion: literal(1), libraryRevision: isRevision, scenarioIds: arrayOf(isUuid),
...
const isPortableArtifact = shape<PortableArtifactDto>({
  schemaVersion: literal(1), artifactName: isText,
  currentRevision: nullable(isRevision), libraryRevision: nullable(isRevision),
});
...
const isOperationContext = union<OperationContextV1>(
  shape<ScenarioOperationContextV1>({
    kind: literal("scenario"), scenarioId: isUuid, expectedRevision: nullable(isRevision),
  }),
  shape<LibraryOperationContextV1>({
    kind: literal("library"), expectedLibraryRevision: nullable(isRevision),
  }),
);
function sameOperationContext(left: OperationContextV1, right: OperationContextV1): boolean {
  return left.kind === "scenario"
    ? right.kind === "scenario" && left.scenarioId === right.scenarioId &&
...
const applicationSettingGuards: { readonly [K in ApplicationSettingKey]: Guard<ApplicationSettingValues[K]> } = {
  appearance: shape<ApplicationSettingValues["appearance"]>({
    theme: optional(choices("system", "light", "dark")), reducedMotion: optional(isBoolean),
  }),
  locale: (value: unknown): value is string =>
    typeof value === "string" && value.length <= 64 && /^[A-Za-z0-9]+(?:-[A-Za-z0-9]+)*$/.test(value),
  units: choices("metric", "us-customary"),
};
function applicationSettingEntry<K extends ApplicationSettingKey>(key: K): Guard<ApplicationSettingEntryV1<K>> {
  return shape<ApplicationSettingEntryV1<K>>({
    value: applicationSettingGuards[key], updatedAt: isTimestamp,
  });
}
...
const settingsSnapshotShape = shape<ApplicationSettingsSnapshotV1>({
  schemaVersion: literal(1), libraryRevision: isRevision, settings: localSettingsShape,
});
...
function settingsWriteGuard(expected: Revision): Guard<ApplicationSettingsWriteResultV1> {
  return (value: unknown): value is ApplicationSettingsWriteResultV1 =>
    boundedJson(value, SETTINGS_COMPACT_BYTES) && settingsWriteShape(value) &&
...
function isChangedSettingEntry(change: SettingsChangeV1): boolean {
  ...
  if (change.key === "appearance") {
    return change.before.value.theme !== change.after.value.theme ||
...
const settingsPreviewShape = shape<SettingsImportPreviewV1>({
  kind: literal("preview"), schemaVersion: literal(1), previewId: isUuid,
  sourceSha256: isDigest, approvalSha256: isDigest, libraryRevision: isRevision,
  changes: arrayOf(isSettingsChange, 3),
});
const isSettingsPreview = (value: unknown): value is SettingsImportPreviewV1 =>
  boundedJson(value, SETTINGS_COMPACT_BYTES) && settingsPreviewShape(value) &&
  value.changes.every((change, index) => isChangedSettingEntry(change) &&
    (index === 0 || (value.changes[index - 1]?.key ?? "") < change.key));
const settingsAppliedShape = shape<SettingsImportAppliedV1>({
  kind: literal("applied"), schemaVersion: literal(1), previewId: isUuid,
  libraryRevision: isRevision, changed: isBoolean,
});
...
const isSettingsDiscarded = shape<SettingsImportDiscardedV1>({
  kind: literal("discarded"), schemaVersion: literal(1),
});
const settingsExportShape = shape<SettingsExportV1>({
  schemaVersion: literal(1), libraryRevision: isRevision,
  writtenBytes: (value: unknown): value is number => isRevision(value) && value > 0 && value <= SETTINGS_COMPACT_BYTES,
});
...
const inventoryChecksumShape = shape<NonNullable<LicenseInventoryPackageV2["checksum"]>>({
  ...
  value: (value: unknown): value is string => typeof value === "string" && /^[0-9a-fA-F]{64}(?:[0-9a-fA-F]{64})?$/.test(value),
});
const isInventoryChecksum = (value: unknown): value is NonNullable<LicenseInventoryPackageV2["checksum"]> =>
...
const isInventoryPackage = shape<LicenseInventoryPackageV2>({
  ecosystem: choices("cargo", "npm"), name: inventoryText(1024), version: inventoryText(1024),
  kind: choices("workspace", "dependency"), licenseConcluded: inventoryText(4096),
  source: inventoryText(1024), checksum: optional(isInventoryChecksum),
});
const inventoryShape = shape<LicenseInventoryV2>({
  scope: literal("lockedWorkspace"), schemaVersion: literal(2),
  ...
  authoritativeInputs: (value: unknown): value is LicenseInventoryV2["authoritativeInputs"] =>
    Array.isArray(value) && value.length === 3 && value[0] === "Cargo.lock" &&
    value[1] === "pnpm-lock.yaml" && value[2] === "xtask/supply-chain-inputs.json",
...
const csvSampleShape = shape<PeopleCsvSampleV1>({
  schemaVersion: literal(1), sourceId: isUuid, dialect: isCsvDialect, record: isCsvRecord,
  cells: nullable(arrayOf(shape({
    text: (value: unknown): value is string => isCsvText(value, 256), truncated: isBoolean,
  }), 64)),
});
const isCsvSample = (value: unknown): value is PeopleCsvSampleV1 =>
  boundedJson(value, 131_072) && csvSampleShape(value) &&
...
async function call<T>(
  ...
  if (options.requestMaximumBytes !== undefined && !boundedJson(request, options.requestMaximumBytes))
...
class OperationScope<C extends OperationContextV1> {
  ...
  run<T>(
    command: SetupOperationCommand | PortableCommand | "settings_import_nonsecret" | "settings_export_nonsecret",
    ...
    const settingsPurpose = purpose.kind === "settingsImportPreview" ||
      purpose.kind === "settingsImportApply" || purpose.kind === "settingsExport";
    const libraryPurpose = settingsPurpose || (portablePurpose &&
      purpose.kind !== "projectExportPreview" && purpose.kind !== "projectExportCreate");
    const libraryRevisionRequired = purpose.kind === "settingsImportApply" ||
      purpose.kind === "projectImportApply" || purpose.kind === "projectRestoreApply" ||
      purpose.kind === "projectBackupCreate";
    if ((this.#context.kind === "library") !== libraryPurpose ||
    ...
    if (settingsPurpose &&
      command !== (purpose.kind === "settingsExport" ? "settings_export_nonsecret" : "settings_import_nonsecret"))
      throw new RangeError("The settings operation requires its matching command");
    if (portablePurpose) {
      if (!isPortablePurpose(purpose.kind) || command !== portableCommands[purpose.kind] ||
        (this.#context.kind === "scenario" && this.#context.expectedRevision === null))
        throw new RangeError("The portable operation requires its matching command and exact context");
    } else if (Object.values(portableCommands).some((portableCommand) => portableCommand === command)) {
    ...
    const setupRequestMaximumBytes =
      purpose.kind === "commandPreview" || purpose.kind === "applyReviewedGeneration"
        ...
        : purpose.kind === "setupView"
          ...
          : purpose.kind === "scenarioSummary" || purpose.kind === "setupStatus" ||
    ...
    if (!boundedJson(payload, nativeRequestMaximumBytes ??
    ...
    const result = (async (): Promise<ApiResponseDto<T>> => {
      ...
      try {
        ...
        const response = await call(
          command,
          portablePurpose ? {
            ...snapshot, schemaVersion: 1, requestId, operationId,
            ...(context.kind === "scenario" ? {
              scenarioId: context.scenarioId, expectedRevision: context.expectedRevision,
            } : libraryRevisionRequired ? { expectedLibraryRevision: context.expectedLibraryRevision } : {}),
          } : context.kind === "scenario" ? {
            ...snapshot, requestId, operationId,
            scenarioId: context.scenarioId, expectedRevision: context.expectedRevision,
          } : purpose.kind === "settingsImportApply" ? {
            previewId: snapshot.previewId, approvalSha256: snapshot.approvalSha256,
            action: "apply", schemaVersion: 1, requestId, operationId,
            expectedLibraryRevision: context.expectedLibraryRevision,
          } : purpose.kind === "settingsImportPreview" ? {
            action: "preview", schemaVersion: 1, requestId, operationId,
...
export class SettingsImportFlow {
  ...
  async #cleanup(preview: SettingsOwnedPreview): Promise<void> {
    ...
    await discardSettingsPreview({
      kind: "creator", operationId, requestId: preview.operation.requestId,
    });
    this.#previews.delete(preview);
  }

  preview(scope: LibraryOperationScope, onProgress?: (event: OperationProgressV1) => void): SetupOperation<SettingsImportPreviewV1> {
    if (this.#disposed) throw inactiveOperation();
    if (this.#previews.size >= 3) throw new RangeError("Dismiss a settings review before opening another");
    const operation = scope.run("settings_import_nonsecret", { kind: "settingsImportPreview" },
      { action: "preview", schemaVersion: 1 }, isSettingsPreview, SETTINGS_WIRE_BYTES, "libraryRevision", onProgress,
    ...
    const result = operation.result.then((response) => {
      ...
      if (preview.id === undefined || preview.closing || this.#disposed || !operation.isCurrent()) {
  ...
  apply(scope: LibraryOperationScope, input: {
    readonly previewId: UuidV7; readonly approvalSha256: string;
  }, onProgress?: (event: OperationProgressV1) => void): SetupOperation<SettingsImportAppliedV1> {
    if (this.#disposed) throw inactiveOperation();
    ...
    const guard = (value: unknown): value is SettingsImportAppliedV1 =>
      isSettingsApplied(value) && value.previewId === previewId &&
      value.libraryRevision === revision + (value.changed ? 1 : 0);
    const operation = scope.run("settings_import_nonsecret", { kind: "settingsImportApply" },
      { action: "apply", schemaVersion: 1, previewId, approvalSha256 },
      guard, SETTINGS_WIRE_BYTES, "libraryRevision", onProgress);
    const result = operation.result.then((response) => {
    ...
    const tracked: SetupOperation<SettingsImportAppliedV1> = {
      requestId: operation.requestId,
      get operationId() { return operation.operationId; },
      result, cancel: () => operation.cancel(), release: () => operation.release(),
      isCurrent: () => !this.#disposed && operation.isCurrent(),
    };
    this.#operations.add(tracked);
    return tracked;
...
export function discardSettingsPreview(target: SettingsPreviewTarget): Promise<ApiResponseDto<SettingsImportDiscardedV1>> {
  ...
  return call("settings_import_nonsecret", {
    action: "discard", schemaVersion: 1, requestId: newUuidV7(), target: { ...target },
  }, isSettingsDiscarded, { maximumBytes: SETTINGS_WIRE_BYTES, revisionKey: null });
}

export function exportNonsecretSettings(
  scope: LibraryOperationScope, onProgress?: (event: OperationProgressV1) => void,
): SetupOperation<SettingsExportV1> {
  return scope.run("settings_export_nonsecret", { kind: "settingsExport" }, { schemaVersion: 1 },
    isSettingsExport, SETTINGS_WIRE_BYTES, "libraryRevision", onProgress);
}
...
export function getLicenseInventory(): Promise<ApiResponseDto<LicenseInventoryV2>> {
  return call("app_get_license_inventory", { requestId: newUuidV7() }, isLicenseInventory,
    { maximumBytes: INVENTORY_WIRE_BYTES, revisionKey: null });
}
...
export class PeopleCsvFlow {
  ...
  async #cleanup(resource: CsvOwnedResource): Promise<void> {
    ...
    await call(resource.kind === "source" ? "people_csv_source_close" : "people_csv_preview_discard", {
      schemaVersion: 1, requestId: newUuidV7(), scenarioId: this.#scenarioId,
  ...
  sample(scope: SetupOperationScope, sourceId: UuidV7,
    ...
    const guard = (value: unknown): value is PeopleCsvSampleV1 =>
      isCsvSample(value) && value.sourceId === sourceId && value.dialect === dialect && value.record === record;
    return this.#track(scope.run("people_csv_record_sample", { kind: "csvSample" },
      { schemaVersion: 1, sourceId, dialect, record }, guard, CSV_SMALL_WIRE_BYTES, null, onProgress));
  }
  ...
  async closeSource(sourceId: UuidV7): Promise<void> {
    await call("people_csv_source_close", { schemaVersion: 1, requestId: newUuidV7(),
  ...
  async discardPreview(previewId: UuidV7): Promise<void> {
    await call("people_csv_preview_discard", { schemaVersion: 1, requestId: newUuidV7(),
  ...
  rejectedRows(previewId: UuidV7): Promise<ApiResponseDto<PeopleCsvRejectedRowsV1>> {
    if (this.#disposed) throw inactiveOperation();
    return call("people_csv_rejected_rows", { schemaVersion: 1, requestId: newUuidV7(),
...
export function getAppPathsSummary(): Promise<ApiResponseDto<AppPathsSummaryDto>> {
  return call("app_get_paths_summary", { requestId: newUuidV7() }, isPaths,
    { maximumBytes: 136 * 1024, revisionKey: null });
}
...
export function listProjects(
  ...
  return call("project_list", { requestId: newUuidV7(), schemaVersion: 1, scope }, arrayOf(isProjectListItem));
}
...
export function openProject(scenarioId: UuidV7): Promise<ApiResponseDto<ProjectListItemV1>> {
  ...
  return call("project_open", { requestId: newUuidV7(), schemaVersion: 1, scenarioId }, guard, {
    maximumBytes: 136 * 1024, revisionKey: "revision",
  });
}
...
export function getProjectMetadata(
  ...
  return call(
    "project_get_metadata",
    { requestId: newUuidV7(), scenarioId },
    isProjectMetadata,
...
export function createProject(input: {
  ...
  return call("project_create", { requestId: newUuidV7(), schemaVersion: 1, ...input }, isProjectMetadata, {
    maximumBytes: 136 * 1024, revisionKey: "revision",
  });
}
...
const isImportOptions = shape<ImportOptions>({
  ...
  includeResults: isBoolean, includeAssets: isBoolean,
});
const isCollisionPlan = shape<CollisionPlan>({
  scenarios: recordOf(choices("create-copy", "replace", "skip"), isUuid),
  supplementalChoices: arrayOf(shape<SupplementalCollisionChoice>({
    section: choices("results", "shared-records", "preferences", "assets"),
    key: isText, action: choices("replace", "skip"),
  })),
});
const isRestoreOrigin = choices("userSelected", "safetyBackups");
const isRestoreAuthorization = shape<RestoreAuthorizationDto>({
  destructiveActionConfirmed: isBoolean, safetyBackupBypassPhrase: nullable(isText),
});
...
export class PortableReviewFlow {
  ...
  #capacity(kind: PortableReviewKind): void {
    ...
    if (count >= 3) throw new RangeError("Dismiss a portable review in this group before opening another");
  }
  ...
  async #cleanup(review: PortableOwnedReview): Promise<void> {
    ...
    await call("project_operation_cancel", {
      schemaVersion: 1, requestId: newUuidV7(),
      target: { kind: "creator", operationId, requestId: review.operation.requestId },
    }, isPortableDiscarded, { maximumBytes: PORTABLE_WIRE_BYTES, revisionKey: null });
    // An acknowledgement is not settlement; retain the creator to repeat cleanup after a late reply.
    if (review.settled && !review.consuming) this.#reviews.delete(review);
  }

  #track<T>(operation: SetupOperation<T>, result: Promise<ApiResponseDto<T>>,
  ...
  #preview<T extends { readonly previewId: UuidV7 }>(
    scope: LibraryOperationScope | SetupOperationScope, kind: PortableReviewKind,
    purpose: PortablePurpose, payload: object, guard: Guard<T>, revisionKey: ResponseRevision<T>,
    libraryRevision: (value: T) => Revision | null,
    onProgress?: (event: OperationProgressV1) => void, replaceLibrary = false,
  ): SetupOperation<T> {
    this.#capacity(kind);
    const operation = scope.run(portableCommands[purpose], { kind: purpose }, payload,
      portableGuard(guard), PORTABLE_WIRE_BYTES, revisionKey, onProgress,
      () => { void this.#cleanup(review).catch(() => undefined); });
    const review: PortableOwnedReview = {
      kind, operation, id: undefined, libraryRevision: null, replaceLibrary,
      ...
      settled: false, consuming: false, consumer: undefined, closing: false,
    };
  ...
  #owned(scope: LibraryOperationScope | SetupOperationScope, previewId: UuidV7,
    kind: PortableReviewKind, expectedLibraryRevision?: Revision,
  ): PortableOwnedReview {
    this.#active();
    const review = [...this.#reviews].find((item) => item.id === previewId);
    if (!isUuid(previewId) || review === undefined || review.kind !== kind ||
      review.closing || review.consuming || !review.settled)
      throw new RangeError("An available review owned by this portable flow is required");
    const context = scope.context;
    if (kind === "export" ? context.kind !== "scenario" ||
        context.scenarioId !== review.scenarioId || context.expectedRevision !== review.scenarioRevision ||
  ...
  #consume<T>(review: PortableOwnedReview, start: () => SetupOperation<T>): SetupOperation<T> {
    ...
    let retained = false;
    const result = operation.result.catch((error: unknown) => {
      const detail = isApiError(error) ? error.details?.portablePreviewRetained : undefined;
      retained = review.kind === "restore" && isApiError(error) &&
        error.code === "restore.safety_backup_failed" &&
        detail?.type === "boolean" && detail.value;
      throw error;
    }).finally(async () => {
      review.consuming = false;
  ...
  previewImport(scope: LibraryOperationScope, options: ImportOptions,
    onProgress?: (event: OperationProgressV1) => void,
  ): SetupOperation<PortablePreviewDto> {
    if (!isImportOptions(options)) throw new RangeError("Valid portable import options are required");
    return this.#preview(scope, "import", "projectImportPreview", { options },
      isPortablePreview, "libraryRevision", (value) => value.libraryRevision, onProgress);
  }

  applyImport(scope: LibraryOperationScope, input: {
    readonly previewId: UuidV7; readonly collisionPlan: CollisionPlan;
  }, onProgress?: (event: OperationProgressV1) => void): SetupOperation<PortableAppliedDto> {
    const review = this.#owned(scope, input.previewId, "import");
    return this.#consume(review, () => {
      if (!isCollisionPlan(input.collisionPlan)) throw new RangeError("A valid collision plan is required");
      return scope.run("project_import_apply", { kind: "projectImportApply" },
        { previewId: input.previewId, collisionPlan: input.collisionPlan },
        portableGuard((value: unknown): value is PortableAppliedDto => isPortableApplied(value) &&
          review.libraryRevision !== null &&
          (value.libraryRevision === review.libraryRevision || value.libraryRevision === review.libraryRevision + 1) &&
          value.safetyBackup.kind === "notRequired"),
        PORTABLE_WIRE_BYTES, "libraryRevision", onProgress,
        () => { void this.#cleanup(review).catch(() => undefined); });
    });
  }

  previewExport(scope: SetupOperationScope,
    onProgress?: (event: OperationProgressV1) => void,
  ): SetupOperation<PortableFilePreviewDto> {
    const revision = requireSetupRevision(scope);
    return this.#preview(scope, "export", "projectExportPreview", {},
      (value: unknown): value is PortableFilePreviewDto => isPortableFilePreview(value) &&
        value.currentRevision === revision && value.backupSummary === null,
      "currentRevision", (value) => value.libraryRevision, onProgress);
  }

  createExport(scope: SetupOperationScope, input: {
    readonly previewId: UuidV7; readonly expectedLibraryRevision: Revision;
  }, onProgress?: (event: OperationProgressV1) => void): SetupOperation<PortableArtifactDto> {
    const review = this.#owned(scope, input.previewId, "export", input.expectedLibraryRevision);
    return this.#consume(review, () => scope.run("project_export_create", { kind: "projectExportCreate" },
      { previewId: input.previewId, expectedLibraryRevision: input.expectedLibraryRevision },
      portableGuard((value: unknown): value is PortableArtifactDto => isPortableArtifact(value) &&
        value.currentRevision === review.scenarioRevision && value.libraryRevision === review.libraryRevision),
      PORTABLE_WIRE_BYTES, "currentRevision", onProgress,
      () => { void this.#cleanup(review).catch(() => undefined); }));
  }

  previewBackup(scope: LibraryOperationScope, title: string,
    onProgress?: (event: OperationProgressV1) => void,
  ): SetupOperation<PortableFilePreviewDto> {
    if (!isText(title)) throw new RangeError("A portable backup title is required");
    return this.#preview(scope, "backup", "projectBackupPreview", { title },
      (value: unknown): value is PortableFilePreviewDto => isPortableFilePreview(value) &&
        value.currentRevision === null && value.backupSummary !== null,
      "libraryRevision", (value) => value.libraryRevision, onProgress);
  }

  createBackup(scope: LibraryOperationScope, previewId: UuidV7,
    onProgress?: (event: OperationProgressV1) => void,
  ): SetupOperation<PortableArtifactDto> {
    const review = this.#owned(scope, previewId, "backup");
    return this.#consume(review, () => scope.run("project_backup_create", { kind: "projectBackupCreate" },
      { previewId },
      portableGuard((value: unknown): value is PortableArtifactDto => isPortableArtifact(value) &&
        value.currentRevision === null && value.libraryRevision === review.libraryRevision),
      PORTABLE_WIRE_BYTES, "libraryRevision", onProgress,
      () => { void this.#cleanup(review).catch(() => undefined); }));
  }

  previewRestore(scope: LibraryOperationScope, options: ImportOptions,
    origin: "userSelected" | "safetyBackups", onProgress?: (event: OperationProgressV1) => void,
  ): SetupOperation<PortablePreviewDto> {
    ...
    return this.#preview(scope, "restore", "projectRestorePreview", { options, origin },
      isPortablePreview, "libraryRevision", (value) => value.libraryRevision, onProgress,
      options.restoreMode === "replace-library");
  }

  applyRestore(scope: LibraryOperationScope, input: {
    readonly previewId: UuidV7; readonly collisionPlan: CollisionPlan;
    ...
    return this.#consume(review, () => {
      ...
      return scope.run("project_restore_apply", { kind: "projectRestoreApply" },
        { previewId: input.previewId, collisionPlan: input.collisionPlan, authorization: input.authorization },
        portableGuard((value: unknown): value is PortableAppliedDto => isPortableApplied(value) &&
          review.libraryRevision !== null &&
          (value.libraryRevision === review.libraryRevision || value.libraryRevision === review.libraryRevision + 1) &&
          (review.replaceLibrary ? value.safetyBackup.kind !== "notRequired" : value.safetyBackup.kind === "notRequired")),
        PORTABLE_WIRE_BYTES, "libraryRevision", onProgress,
        () => { void this.#cleanup(review).catch(() => undefined); });
    });
  }

  inspectUnopenedBundle(scope: LibraryOperationScope,
    onProgress?: (event: OperationProgressV1) => void,
  ): SetupOperation<UnopenedBundlePreviewDto> {
    return this.#preview(scope, "unopened", "projectUnopenedBundleInspect", {},
      isUnopenedPreview, null, () => null, onProgress);
  }

  reexportUnopenedBundle(scope: LibraryOperationScope, previewId: UuidV7,
    onProgress?: (event: OperationProgressV1) => void,
  ): SetupOperation<PortableArtifactDto> {
    const review = this.#owned(scope, previewId, "unopened");
    return this.#consume(review, () => scope.run("project_unopened_bundle_reexport",
      { kind: "projectUnopenedBundleReexport" }, { previewId },
      portableGuard((value: unknown): value is PortableArtifactDto => isPortableArtifact(value) &&
        value.currentRevision === null && value.libraryRevision === null),
      PORTABLE_WIRE_BYTES, null, onProgress,
      () => { void this.#cleanup(review).catch(() => undefined); }));
  }
...
export function getApplicationSettings(): Promise<ApiResponseDto<ApplicationSettingsSnapshotV1>> {
  return call("settings_get", { requestId: newUuidV7(), schemaVersion: 1 }, isSettingsSnapshot, {
    maximumBytes: SETTINGS_WIRE_BYTES, revisionKey: "libraryRevision",
  });
}
...
export function updateSetting<K extends ApplicationSettingKey>(
  ...
  return call("settings_update", {
    requestId: newUuidV7(), schemaVersion: 1, key, value, expectedLibraryRevision,
...
export function resetSettingsSection(
  ...
  return call("settings_reset_section", {
    requestId: newUuidV7(), schemaVersion: 1, key, expectedLibraryRevision,
  }, settingsWriteGuard(expectedLibraryRevision), {
    maximumBytes: SETTINGS_WIRE_BYTES, revisionKey: "libraryRevision",
  });
}
...
fn generated_rust_command_catalog() -> String {
    ...
    output.push_str(
        r"
// build.rs consumes only REGISTERED_COMMANDS from this shared generated source.
#[allow(unused_macros)]
...
mod tests {
    ...
    #[test]
    fn foundation_capability_is_confined_to_the_configured_local_window() -> anyhow::Result<()> {
        let config: serde_json::Value =
            serde_json::from_str(include_str!("../../apps/desktop/src-tauri/tauri.conf.json"))?;
        ...
        let windows = config["app"]["windows"]
            .as_array()
            .ok_or_else(|| anyhow::anyhow!("configured windows must be an array"))?
            .iter()
            .map(|window| window["label"].clone())
            .collect::<Vec<_>>();
        assert_eq!(windows, [serde_json::json!("main")]);
        assert_eq!(capability["windows"], serde_json::json!(windows));
