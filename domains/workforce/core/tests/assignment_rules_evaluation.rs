...
fn rest_rule(value: &mut Value, index: u32, minimum_minutes: u32) {
    ...
    value["domain"]["rules"][id(index)]["afterScope"] =
        json!({"people":{"kind":"all"}});
    value["domain"]["rules"][id(index)]["beforeScope"] =
        json!({"people":{"kind":"all"}});
...
fn rest_evidence(
    ...
    assert_eq!(
        record.observed.get(&VerificationFactId::new("official.workforce.fact.witness_reason")?),
    ...
    for (key, shift) in [
        ...
        assert_eq!(entity.id.as_str(), format!("official.workforce.{}", id(shift)));
        assert!(record.affected_entities.contains(entity));
    }
    assert_eq!(
        record.expected.get(&VerificationFactId::new("official.workforce.fact.required_rest_minutes")?),
...
fn rest_uses_exact_elapsed_threshold_and_directional_evidence() -> Result {
    ...
    for (start, violations) in [
        ...
        times(&mut value["domain"]["entities"][id(30)], start, "2026-11-01T21:00:00");
        let result = run(&value, &[(1, 30), (1, 8)])?;
        assert_eq!(totals(&result, 20)?, (1, violations));
        assert_eq!(result.evaluations, run(&value, &[(1, 8), (1, 30)])?.evaluations);
        if violations != 0 {
...
fn rest_role_chronology_is_not_uuid_or_selection_order() -> Result {
    ...
    assert_eq!(forward.evaluations, run(&value, &[(1, 30), (1, 8)])?.evaluations);
    value["domain"]["rules"][id(20)]["afterScope"]["categories"] = json!(["clinic"]);
    value["domain"]["rules"][id(20)]["beforeScope"]["categories"] = json!(["call"]);
    assert_eq!(totals(&run(&value, &[(1, 8), (1, 30)])?, 20)?, (0, 0));

    times(&mut value["domain"]["entities"][id(30)], "2026-11-01T08:00:00", "2026-11-01T09:00:00");
...
fn rest_overlap_violates_zero_even_when_no_overlap_is_compatible() -> Result {
    ...
    another_shift(&mut value, 30, "2026-11-01T09:59:59.999999999", "2026-11-01T11:00:00");
    let result = run(&value, &[(1, 8), (1, 30)])?;
    assert_eq!(totals(&result, 20)?, (1, 1));
    assert_eq!(totals(&result, 21)?, (1, 0));
    rest_evidence(&result, 20, 8, 30, 0, 0, -1)?;
    times(&mut value["domain"]["entities"][id(30)], "2026-11-01T10:00:00", "2026-11-01T11:00:00");
    assert_eq!(totals(&run(&value, &[(1, 8), (1, 30)])?, 20)?, (1, 0));
    Ok(())
}
...
fn rest_counts_nonadjacent_pairs_without_an_intervening_reset() -> Result {
    ...
    times(&mut value["domain"]["entities"][id(8)], "2026-11-01T08:00:00", "2026-11-01T18:00:00");
...
fn rest_intersects_every_common_and_directional_scope_dimension() -> Result {
    ...
    value["domain"]["entities"][id(35)] = json!({"kind":"location","id":id(35),"name":"Other","transitions":[]});
    let full_scope = json!({"people":{"kind":"selected","personIds":[id(1)]},"teamIds":[id(30)],"assignmentTypeIds":[id(4)],"categories":["clinic"],"weekdays":["sunday"],"locationIds":[id(5)]});
    ...
    for role in ["scope", "afterScope", "beforeScope"] {
        for (field, replacement) in [
            ...
            ("people", json!({"kind":"filter","allTags":["absent"],"anyTags":[]})),
            ("people", json!({"kind":"filter","allTags":[],"anyTags":["absent"]})),
            ("teamIds", json!([id(31)])),
    ...
    value["domain"]["rules"][id(20)]["scope"] = json!({"people":{"kind":"all"},"assignmentTypeIds":[id(34)]});
    assert_eq!(totals(&run(&value, &[(1, 8), (1, 40)])?, 20)?, (0, 0));
    value["domain"]["rules"][id(20)]["active"] = json!(false);
    value["domain"]["rules"][id(20)]["afterScope"]["people"] = json!({"kind":"selected","personIds":[]});
    ...
    let document: ScenarioDocument = serde_json::from_value(value)?;
    assert!(matches!(evaluate_assignment_rules(&document, &[], None), Err(AssignmentRuleError::InvalidDocument(_))));
    Ok(())
}
...
fn rest_sees_identified_inadmissible_selections_and_empty_selections() -> Result {
    ...
    value["domain"]["entities"][id(1)]["eligibleAssignmentTypeIds"] = json!([]);
    availability(&mut value, 31, "unavailable", "2026-11-01T11:00:00Z", "2026-11-01T12:00:00Z");
    availability(&mut value, 32, "approvedTimeOff", "2026-11-01T11:00:00Z", "2026-11-01T12:00:00Z");
    let result = run(&value, &[(1, 8), (1, 30)])?;
    assert_eq!(totals(&result, 20)?, (1, 1));
...
fn rest_distinguishes_overnight_dst_elapsed_time_from_reporting_weekday() -> Result {
    for (date, next, source_start, source_end, target_start, target_end, offset, next_offset, violations, seconds) in [
        ("2026-03-07", "2026-03-08", "2026-03-08T04:00:00Z", "2026-03-08T06:00:00Z", "2026-03-08T15:00:00Z", "2026-03-08T16:00:00Z", -18000, -14400, 1, 32400),
        ("2026-10-31", "2026-11-01", "2026-11-01T03:00:00Z", "2026-11-01T05:00:00Z", "2026-11-01T16:00:00Z", "2026-11-01T17:00:00Z", -14400, -18000, 0, 39600),
    ] {
        let mut value = base()?;
        value["settings"]["timeZone"] = json!("America/New_York");
        value["settings"]["overlapPolicy"] = json!("earlier");
        value["settings"]["horizon"] = json!({"start":source_start,"end":target_end});
        rest_rule(&mut value, 20, 600);
        another_shift(&mut value, 30, "2026-11-01T11:00:00", "2026-11-01T12:00:00");
        for (shift, start, end, local_start, local_end, shift_offset) in [
            (8, source_start, source_end, format!("{date}T23:00:00"), format!("{next}T01:00:00"), offset),
            (30, target_start, target_end, format!("{next}T11:00:00"), format!("{next}T12:00:00"), next_offset),
        ] {
            value["domain"]["entities"][id(shift)]["startsAt"] = json!({"instant":start,"local":local_start,"offsetSeconds":shift_offset});
            value["domain"]["entities"][id(shift)]["endsAt"] = json!({"instant":end,"local":local_end,"offsetSeconds":shift_offset});
        }
        value["domain"]["entities"][id(8)]["reportingAttribution"] = json!("endLocalDate");
        for role in ["scope", "afterScope", "beforeScope"] {
        ...
        rest_evidence(&run(&value, &[(1, 8), (1, 30)])?, 20, 8, 30, 700, seconds, 0)?;
...
fn rest_maximum_minutes_and_extreme_instants_do_not_add_overflowing_endpoints() -> Result {
    ...
    value["settings"]["horizon"] = json!({"start":"0001-01-01T00:00:00Z","end":"9998-12-31T23:00:00Z"});
    times(&mut value["domain"]["entities"][id(8)], "0001-01-01T00:00:00", "0001-01-02T00:00:00");
    another_shift(&mut value, 30, "9998-12-31T20:00:00", "9998-12-31T21:00:00");
    assert_eq!(totals(&run(&value, &[(1, 8), (1, 30)])?, 20)?, (1, 0));
    // Near the timestamp ceiling, adding the required minutes to the source end would fail.
    times(&mut value["domain"]["entities"][id(8)], "9998-12-31T18:00:00", "9998-12-31T19:00:00");
    ...
    times(&mut value["domain"]["entities"][id(8)], "0001-01-01T00:00:00", "9998-12-31T19:00:00.000000001");
    times(&mut value["domain"]["entities"][id(30)], "0001-01-02T00:00:00", "0001-01-03T00:00:00");
    let negative = run(&value, &[(1, 8), (1, 30)])?;
    assert_eq!(totals(&negative, 20)?, (1, 1));
    let seconds = "0001-01-02T00:00:00Z".parse::<jiff::Timestamp>()?.as_second()
        - "9998-12-31T19:00:00Z".parse::<jiff::Timestamp>()?.as_second();
...
fn rest_safe_suffix_counts_quadratic_predicates_without_quadratic_work_or_output() -> Result {
    ...
    value["settings"]["horizon"] = json!({"start":"2026-11-01T00:00:00Z","end":"2026-11-09T00:00:00Z"});
    value["domain"]["entities"].as_object_mut().ok_or("entities")?.remove(&id(8));
    let prototype = base()?["domain"]["entities"][id(8)].clone();
    let mut pairs = Vec::new();
    for index in 0..7000_u32 {
        let mut shift = prototype.clone();
        let day = 1 + index / 960;
        let second = index % 960 * 90;
        let start = format!("2026-11-{day:02}T{:02}:{:02}:{:02}", second / 3600, second / 60 % 60, second % 60);
        let end_second = second + 30;
        let end = format!("2026-11-{day:02}T{:02}:{:02}:{:02}", end_second / 3600, end_second / 60 % 60, end_second % 60);
        shift["id"] = json!(id(1000 + index));
        times(&mut shift, &start, &end);
        value["domain"]["entities"][id(1000 + index)] = shift;
        pairs.push((1, 1000 + index));
    ...
    let result = run(&value, &pairs)?;
    // More predicates than the entire work-step ceiling: a charged quadratic safe walk fails.
    assert_eq!(totals(&result, 20)?, (24_496_500, 0));
    assert_eq!(result.evaluations.len(), 2);
    Ok(())
}
