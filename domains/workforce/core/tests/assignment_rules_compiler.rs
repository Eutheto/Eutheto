...
use eutheto_workforce::{
    assignment_rules::{AssignmentRuleCompilation, AssignmentRuleError, AssignmentRuleLimit, RejectionCause, analyze_assignments, compile_assignment_rules},
...
fn entity(document: &mut ScenarioDocument, index: u32) -> Result<&mut Value> {
    document.domain.entities.get_mut(&id(index).parse()?).ok_or_else(|| "missing fixture entity".into())
}
...
fn rule(document: &mut ScenarioDocument, index: u32, kind: &str) -> Result {
    ...
    if kind == "noOverlap" { value["compatibleCategoryPairs"] = json!([]); }
    document.domain.rules.insert(id(index).parse()?, value);
    Ok(())
}

fn context(limits: PlanningIrLimitsV1) -> CompileContext {
    CompileContext { scenario_revision: 1, semantic_metadata: BTreeMap::new(), cancellation: CancellationToken::new(), planning_limits: limits }
}

fn compile(document: &ScenarioDocument) -> Result<AssignmentRuleCompilation> {
    Ok(compile_assignment_rules(document, &context(PlanningIrLimitsV1::DEFAULT))?)
}

fn pair(person: u32, shift: u32) -> Result<AssignmentPair> { Ok(AssignmentPair { person_id: id(person).parse()?, shift_id: id(shift).parse()? }) }
...
fn allows(model: &AssignmentRuleCompilation, selected: &[AssignmentPair]) -> bool {
    if selected.iter().any(|pair| !model.variables.iter().any(|entry| entry.pair == *pair)) { return false; }
    let values: BTreeMap<_, _> = model.variables.iter().map(|entry| (&entry.variable.id, selected.contains(&entry.pair))).collect();
    model.constraints.iter().all(|record| {
        let satisfied = |literal: &eutheto_planning_ir::Literal| values.get(&literal.variable).is_some_and(|value| *value == literal.positive);
        match &record.body {
            Constraint::BoolOr { literals } => literals.iter().any(satisfied),
            Constraint::AtMostOne { literals } => literals.iter().filter(|literal| satisfied(literal)).count() <= 1,
...
fn availability(document: &mut ScenarioDocument, index: u32, kind: &str, start: &str, end: &str) -> Result {
...
fn eligibility_activation_and_scope_own_both_predicates() -> Result {
    ...
    assert!(analysis.rejections.iter().any(|rejection| matches!(rejection.cause, RejectionCause::AssignmentTypeNotAllowed { .. })));
    assert!(analysis.rejections.iter().any(|rejection| matches!(rejection.cause, RejectionCause::QualificationExpression { .. })));
    value.domain.rules.get_mut(&id(20).parse()?).ok_or("missing rule")?["scope"]["people"] = json!({"kind":"filter","allTags":["absent"],"anyTags":[]});
    assert!(allows(&compile(&value)?, &[pair(1, 7)?]));
    value.domain.rules.get_mut(&id(20).parse()?).ok_or("missing rule")?["active"] = json!(false);
...
fn adjacent_qualification_renewals_cover_but_nanosecond_gap_and_expiry_do_not() -> Result {
    ...
    entity(&mut value, 1)?["qualificationGrants"][1]["effectiveFrom"] = json!("2026-11-01T06:00:00.000000001Z");
    assert!(compile(&value)?.variables.is_empty());
    entity(&mut value, 1)?["qualificationGrants"] = json!([{"qualificationId":id(11),"expiresAt":"2026-11-01T05:30:00Z"}]);
    assert!(compile(&value)?.variables.is_empty());
    Ok(())
}
...
fn any_qualification_does_not_allow_alternating_partial_credentials() -> Result {
    let mut value = document()?;
    value.domain.entities.insert(id(12).parse()?, json!({"kind":"qualification","id":id(12),"name":"Other","description":""}));
    entity(&mut value, 4)?["qualifications"] = json!({"kind":"matches","allQualificationIds":[],"anyQualificationIds":[id(11),id(12)]});
    entity(&mut value, 1)?["qualificationGrants"] = json!([
        {"qualificationId":id(11),"expiresAt":"2026-11-01T06:00:00Z"},
...
fn activity_covers_the_complete_post_horizon_tail_without_authored_rules() -> Result {
    ...
    entity(&mut value, 6)?["timing"] = json!({"kind":"localWindow","startTime":"23:00:00","endTime":"02:00:00","endDayOffset":1});
    entity(&mut value, 1)?["activeRange"] = json!({"kind":"dateRange","startDate":"2026-01-01","endDateExclusive":"2026-11-02"});
    let analysis = analyze_assignments(&value, None, PlanningIrLimitsV1::DEFAULT)?;
    assert!(analysis.candidates.is_empty());
    assert!(matches!(analysis.rejections[0].cause, RejectionCause::OutsideActiveRange { outside, .. } if outside.start == "2026-11-02T05:00:00Z".parse()? && outside.end == "2026-11-02T07:00:00Z".parse()?));
...
fn ordinary_records_are_conjunctive_and_approved_leave_is_unconditional() -> Result {
    let mut value = document()?;
    availability(&mut value, 30, "availableOnly", "2026-11-01T05:00:00Z", "2026-11-01T07:00:00Z")?;
    availability(&mut value, 31, "availableOnly", "2026-11-01T06:00:00Z", "2026-11-01T08:00:00Z")?;
    assert!(allows(&compile(&value)?, &[pair(1, 7)?]));
    rule(&mut value, 21, "availability")?;
    assert!(compile(&value)?.variables.is_empty());
    value.domain.rules.clear();
    availability(&mut value, 32, "requestedTimeOff", "2026-11-01T05:30:00Z", "2026-11-01T06:30:00Z")?;
    assert!(allows(&compile(&value)?, &[pair(1, 7)?]));
    ...
    assert!(result.rejections.iter().all(|rejection| rejection.binding_id == leave_id));
    entity(&mut value, 32)?["locationIds"] = json!([id(5)]);
    entity(&mut value, 32)?["effectiveRange"] = json!({"startDate":"2026-11-02","endDateExclusive":"2026-11-03"});
    assert!(allows(&compile(&value)?, &[pair(1, 7)?]));
    Ok(())
}
...
fn weekly_window_union_includes_pre_effective_overnight_starts() -> Result {
    let mut value = document()?;
    rule(&mut value, 21, "availability")?;
    availability(&mut value, 30, "availableOnly", "2026-11-01T05:00:00Z", "2026-11-01T08:00:00Z")?;
    entity(&mut value, 30)?["effectiveRange"] = json!({"startDate":"2026-11-01","endDateExclusive":"2026-11-02"});
...
fn all_source_owned_rejection_causes_survive_earlier_failures() -> Result {
    ...
    rule(&mut value, 20, "eligibility")?;
    rule(&mut value, 21, "availability")?;
    entity(&mut value, 1)?["activeRange"] = json!({"kind":"dateRange","startDate":"2026-11-02","endDateExclusive":"2026-12-01"});
    entity(&mut value, 1)?["eligibleAssignmentTypeIds"] = json!([]);
    entity(&mut value, 1)?["qualificationGrants"] = json!([]);
    availability(&mut value, 30, "unavailable", "2026-11-01T05:00:00Z", "2026-11-01T08:00:00Z")?;
    availability(&mut value, 31, "availableOnly", "2026-11-01T08:00:00Z", "2026-11-01T09:00:00Z")?;
    availability(&mut value, 32, "approvedTimeOff", "2026-11-01T05:00:00Z", "2026-11-01T08:00:00Z")?;
    let result = compile(&value)?;
    assert!(result.variables.is_empty());
    ...
    for index in [1, 20, 21, 32] {
        ...
        counts.insert(index, result.rejections.iter().filter(|rejection| rejection.binding_id == binding).count());
    }
...
fn coverage_normalizes_empty_and_excess_populations_into_valid_primitives() -> Result {
    ...
    for (coverage, none, one, false_primitive) in [
        (json!({"kind":"exact","count":0,"qualificationMinimums":[]}), true, false, false),
        (json!({"kind":"exact","count":1,"qualificationMinimums":[]}), false, true, false),
        (json!({"kind":"exact","count":2,"qualificationMinimums":[]}), false, false, true),
        (json!({"kind":"atLeast","minimum":0,"maximumCount":9,"qualificationMinimums":[]}), true, true, false),
        (json!({"kind":"atLeast","minimum":1,"qualificationMinimums":[]}), false, true, false),
        (json!({"kind":"atLeast","minimum":2,"qualificationMinimums":[]}), false, false, true),
    ] {
        entity(&mut value, 6)?["coverage"] = coverage;
        let result = compile(&value)?;
        assert_eq!(allows(&result, &[]), none);
        assert_eq!(allows(&result, &[pair(1, 7)?]), one);
        assert_eq!(matches!(&result.constraints[0].body, Constraint::BoolOr { literals } if literals.is_empty()), false_primitive);
    }
    value.domain.rules.get_mut(&id(22).parse()?).ok_or("missing rule")?["scope"]["people"] = json!({"kind":"filter","allTags":["absent"],"anyTags":[]});
    entity(&mut value, 6)?["coverage"] = json!({"kind":"exact","count":0,"qualificationMinimums":[]});
...
fn qualification_minima_share_people_but_never_merge_distinct_owners() -> Result {
    ...
    rule(&mut value, 22, "coverage")?;
    value.domain.entities.insert(id(12).parse()?, json!({"kind":"qualification","id":id(12),"name":"Other","description":""}));
    entity(&mut value, 1)?["qualificationGrants"] = json!([{"qualificationId":id(11)},{"qualificationId":id(12)}]);
    ...
    assert!(missing.validation.issues.iter().any(|issue| issue.code == "official.workforce.qualification_shortage"));
    Ok(())
}
...
fn overlap_compatibility_is_rule_owned_and_touching_is_not_overlap() -> Result {
    ...
    assert!(!allows(&compile(&value)?, &[pair(1, 7)?, pair(1, 8)?]));
    value.domain.rules.get_mut(&id(23).parse()?).ok_or("missing rule")?["compatibleCategoryPairs"] = json!([{"firstCategory":"clinic","secondCategory":"clinic"}]);
    assert!(allows(&compile(&value)?, &[pair(1, 7)?, pair(1, 8)?]));
...
fn coverage_intersects_reporting_weekdays_with_local_start_dates() -> Result {
    ...
    entity(&mut value, 6)?["coverage"] = json!({"kind":"exact","count":0,"qualificationMinimums":[]});
    rule(&mut value, 22, "coverage")?;
    value.domain.rules.get_mut(&id(22).parse()?).ok_or("missing rule")?["scope"]["weekdays"] = json!(["monday"]);
    ...
    assert!(!allows(&result, &[pair(1, 7)?]));
    assert!(result.validation.issues.iter().any(|issue| issue.code == "official.workforce.contradictory_coverage_bounds"));
    Ok(())
}
...
fn hard_lock_readiness_and_partition_exclude_soft_and_unlocked_states() -> Result {
    ...
    for (index, shift) in [(50, 7), (51, 8)] {
        value.domain.locked_assignments.insert(id(index).parse()?, json!({"id":id(index),"personId":id(1),"shiftId":id(shift),"state":{"kind":"hard"}}));
    }
    let hard = compile(&value)?;
    assert_eq!(hard.obligations.remaining, vec![id(50).parse()?, id(51).parse()?]);
    assert!(hard.validation.issues.iter().any(|issue| issue.code == "official.workforce.hard_lock_overlap"));
    value.domain.locked_assignments.get_mut(&id(50).parse()?).ok_or("missing lock")?["state"] = json!({"kind":"soft","stabilityWeight":1});
    value.domain.locked_assignments.get_mut(&id(51).parse()?).ok_or("missing lock")?["state"] = json!({"kind":"unlocked"});
    let nonhard = compile(&value)?;
    assert!(nonhard.obligations.remaining.is_empty());
    assert!(!nonhard.validation.issues.iter().any(|issue| issue.code.starts_with("official.workforce.hard_lock")));
    value.domain.locked_assignments.get_mut(&id(50).parse()?).ok_or("missing lock")?["state"] = json!({"kind":"hard"});
    entity(&mut value, 6)?["recurrence"]["excludedDates"] = json!(["2026-11-01"]);
    let dormant = compile(&value)?;
    assert!(dormant.validation.issues.iter().any(|issue| issue.code == "official.workforce.hard_lock_unresolved_shift"));
    Ok(())
}
...
fn rejected_hard_locks_and_excess_hard_coverage_are_readiness_only() -> Result {
    ...
    entity(&mut value, 6)?["coverage"] = json!({"kind":"exact","count":0,"qualificationMinimums":[]});
    entity(&mut value, 1)?["qualificationGrants"] = json!([]);
    value.domain.locked_assignments.insert(id(50).parse()?, json!({"id":id(50),"personId":id(1),"shiftId":id(7),"state":{"kind":"hard"}}));
    let result = compile(&value)?;
    assert!(result.validation.issues.iter().any(|issue| issue.code == "official.workforce.hard_lock_rejected_pair"));
    assert!(result.validation.issues.iter().any(|issue| issue.code == "official.workforce.hard_locked_coverage_excess"));
...
fn provenance_is_reachable_source_owned_and_has_explicit_pair_association() -> Result {
    ...
    rule(&mut value, 23, "noOverlap")?;
    let result = compile(&value)?;
    let mut reachable: BTreeSet<_> = result.variables.iter().map(|entry| entry.variable.provenance.clone()).chain(result.constraints.iter().map(|record| record.provenance.clone())).collect();
    for record in &result.provenance {
        if let Some(parent) = &record.parent { reachable.insert(parent.clone()); }
    }
    assert_eq!(reachable, result.provenance.iter().map(|record| record.id.clone()).collect());
    assert!(result.variables.windows(2).all(|pair| pair[0].variable.id < pair[1].variable.id));
    assert_eq!(result.variables.iter().map(|entry| entry.pair).collect::<BTreeSet<_>>(), BTreeSet::from([pair(1, 7)?, pair(1, 8)?]));
    ...
    assert_eq!(result.estimate.provenance_records, result.provenance.len() as u64);
...
fn malformed_cancellation_and_tight_variable_constraint_limits_fail_atomically() -> Result {
    ...
    assert!(matches!(compile_assignment_rules(&value, &context(limits)), Err(AssignmentRuleError::LimitExceeded(AssignmentRuleLimit::Variables))));
    rule(&mut value, 22, "coverage")?;
    limits = PlanningIrLimitsV1::DEFAULT;
    limits.max_constraints = 1;
    assert!(matches!(compile_assignment_rules(&value, &context(limits)), Err(AssignmentRuleError::LimitExceeded(AssignmentRuleLimit::Constraints))));
    let cancelled = context(PlanningIrLimitsV1::DEFAULT);
    cancelled.cancellation.cancel();
    assert_eq!(compile_assignment_rules(&value, &cancelled), Err(AssignmentRuleError::Cancelled));
    assert_eq!(analyze_assignments(&value, Some(&cancelled.cancellation), PlanningIrLimitsV1::DEFAULT), Err(AssignmentRuleError::Cancelled));
    value.domain.rules.get_mut(&id(22).parse()?).ok_or("missing rule")?["scope"]["people"] = json!({"kind":"selected","personIds":[]});
    assert!(matches!(compile_assignment_rules(&value, &context(PlanningIrLimitsV1::DEFAULT)), Err(AssignmentRuleError::InvalidDocument(_))));
    Ok(())
}
...
fn aggregate_byte_record_and_reference_boundaries_do_not_truncate_rejections() -> Result {
    let mut value = document()?;
    rule(&mut value, 21, "availability")?;
    for index in 30..38 { availability(&mut value, index, "unavailable", "2026-11-01T05:00:00Z", "2026-11-01T08:00:00Z")?; }
    for field in 0..3 {
        let set = |cap| {
            let mut limits = PlanningIrLimitsV1::DEFAULT;
            match field { 0 => limits.max_ir_bytes = cap, 1 => limits.max_provenance_records = cap, _ => limits.max_total_refs = cap }
            limits
        };
        let mut low = 0;
        let mut high = match field { 0 => PlanningIrLimitsV1::DEFAULT.max_ir_bytes, 1 => PlanningIrLimitsV1::DEFAULT.max_provenance_records, _ => PlanningIrLimitsV1::DEFAULT.max_total_refs };
        while low + 1 < high {
            let middle = low + (high - low) / 2;
            if compile_assignment_rules(&value, &context(set(middle))).is_ok() { high = middle; } else { low = middle; }
        }
        ...
        assert!(matches!(compile_assignment_rules(&value, &context(set(high - 1))), Err(AssignmentRuleError::LimitExceeded(_))));
    }
    Ok(())
}
...
fn overlap_requires_both_shifts_to_match_every_filter() -> Result {
    ...
    rule(&mut value, 23, "noOverlap")?;
    value.domain.rules.get_mut(&id(23).parse()?).ok_or("missing rule")?["scope"]["assignmentTypeIds"] = json!([id(4)]);
    assert!(allows(&compile(&value)?, &[pair(1, 7)?, pair(1, 8)?]));
    value.domain.rules.get_mut(&id(23).parse()?).ok_or("missing rule")?["scope"]["assignmentTypeIds"] = json!([id(4), id(41)]);
    assert!(!allows(&compile(&value)?, &[pair(1, 7)?, pair(1, 8)?]));
    value.domain.rules.get_mut(&id(23).parse()?).ok_or("missing rule")?["scope"]["categories"] = json!(["clinic"]);
    assert!(allows(&compile(&value)?, &[pair(1, 7)?, pair(1, 8)?]));
    Ok(())
}
...
fn raw_cartesian_limit_precedes_candidate_allocation() -> Result {
    ...
    for index in 0..999 {
        ...
        value.domain.entities.insert(id(3000 + index).parse()?, record);
    }
    let mut limits = PlanningIrLimitsV1::DEFAULT;
    limits.max_variables = 0;
    assert_eq!(analyze_assignments(&value, None, limits), Err(AssignmentRuleError::LimitExceeded(AssignmentRuleLimit::InspectedPairs)));
    Ok(())
}
...
fn qualification_lower_bound_conflicts_with_an_independent_owner_upper_bound() -> Result {
    ...
    value.domain.entities.insert(id(40).parse()?, json!({"kind":"coverageRequirement","id":id(40),"active":true,"scope":{"kind":"all"},
        "coverage":{"kind":"exact","count":0,"qualificationMinimums":[]}}));
    let result = compile(&value)?;
    assert!(!allows(&result, &[]));
    assert!(!allows(&result, &[pair(1, 7)?]));
    assert!(result.validation.issues.iter().any(|issue| issue.code == "official.workforce.qualification_above_headcount"
        && issue.message.contains(&id(6)) && issue.message.contains(&id(40))));
    Ok(())
}
