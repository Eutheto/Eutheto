...
use jiff::{SignedDuration, Span, civil::{Date, DateTime}, tz::TimeZone};
...
pub(crate) fn issue(
    ...
    TemporalError::Issue(TemporalIssue { kind, entity_id, local_date })
}
...
pub(crate) fn resolve_endpoint(
    ...
    ).map_err(|error| issue(TemporalIssueKind::Resolution(error.kind), Some(entity_id), Some(local.date())))
}
...
pub(crate) fn checked_interval(
    ...
    if starts_at.instant >= ends_at.instant {
        return Err(issue(TemporalIssueKind::InvalidInterval, Some(entity_id), Some(starts_at.local.as_datetime().date())));
    }
...
pub(crate) fn resolve_interval(
    ...
    checked_interval(resolve_endpoint(start, settings, entity_id)?, resolve_endpoint(end, settings, entity_id)?, entity_id)
}
...
pub(crate) fn resolve_shift_timing(
    ...
    match timing {
        ShiftTiming::LocalWindow { start_time, end_time, end_day_offset } => {
            let end_date = date.checked_add(Span::new().days(i64::from(end_day_offset))).map_err(|_| overflow())?;
            resolve_interval(date.to_datetime(start_time), end_date.to_datetime(end_time), settings, entity_id)
        }
        ShiftTiming::ElapsedDuration { start_time, duration_minutes } => {
            ...
            let instant = start.instant.as_timestamp().checked_add(SignedDuration::from_mins(i64::from(duration_minutes))).map_err(|_| overflow())?;
            let zone = TimeZone::get(settings.time_zone.as_str()).map_err(|_| overflow())?;
            let end = instant.to_zoned(zone);
            checked_interval(start, ResolvedLocalTime {
