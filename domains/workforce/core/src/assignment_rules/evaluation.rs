...
use crate::{
    ids::ShiftId,
    model::{
        ActiveRange, AssignmentPair, AvailabilityKind, CategoryPair, LockState, Scope,
    ...
    for person_id in &input.people {
        ...
        if let Some(shifts) = selected.by_person.get(person_id) {
            let allowed = match person.active_range {
                ...
                ActiveRange::DateRange(range) => Some(date_range(
                    range,
                    &document.settings,
                    EntityId::from_uuid(person_id.as_uuid()),
                )?),
            };
            for shift in shifts {
                budget.step()?;
                ...
                let failed = allowed
                    .is_some_and(|allowed| query.start < allowed.start || query.end > allowed.end);
        ...
        if let Some(shifts) = selected.by_person.get(&record.person_id) {
            for shift in shifts {
                ...
                if predicates::availability_matches(record, &metadata, &mut budget)? {
                    let failure = predicates::availability_failure(
                        record,
                        shift,
                        &document.settings,
                        &mut budget,
        ...
        match rule {
            WorkforceRule::Eligibility { .. } | WorkforceRule::Availability { .. } => {
                for (person_id, shifts) in &selected.by_person {
                    budget.step()?;
                    let person = input.person(*person_id).ok_or_else(invalid)?;
                    if !predicates::person_matches(person, scope, &mut budget)? {
                        continue;
                    }
                    for shift in shifts {
                        budget.step()?;
                        let metadata = input.metadata(shift)?;
                        if !predicates::shift_matches(shift, &metadata, scope, &mut budget)? {
                            continue;
                        }
                        ...
                        if matches!(rule, WorkforceRule::Eligibility { .. }) {
                            ...
                            summary.predicate(
                                !allowed,
                                predicates::pair_witness(
                                    &input,
                                    pair,
                                    "assignment_type_not_allowed",
                                )?,
                                &mut budget,
                            )?;
                            ...
                            for record_id in records {
                                ...
                                ) || !predicates::availability_matches(
                                    record,
                                    &metadata,
                                    &mut budget,
                                )? {
                                ...
                                let reason = if record.availability_kind
                                    == AvailabilityKind::AvailableOnly
                                {
                                    "outside_available_only"
                                } else {
                                    "unavailable"
                                };
                                let mut witness = Witness::pair(
            ...
            WorkforceRule::Coverage { .. } => {
                coverage::evaluate(&input, &selected, scope, &mut summary, &mut budget)?
            }
