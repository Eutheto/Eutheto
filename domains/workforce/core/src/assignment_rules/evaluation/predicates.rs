...
use super::{Witness, invalid};
use crate::{
    ids::QualificationId,
    model::{
        Availability, AvailabilityKind, Person, PersonSelection, QualificationExpression,
...
pub(super) fn shift_matches(
    ...
    budget.step()?;
    Ok(restriction(
        scope.assignment_type_ids.as_deref(),
        Some(&metadata.assignment_type.id),
        budget,
    )? && restriction(
        scope.categories.as_deref(),
        Some(&metadata.assignment_type.category),
        budget,
    )? && restriction(
        scope.weekdays.as_deref(),
        Some(&weekday(shift.reporting_date)),
        budget,
    )? && restriction(
        scope.location_ids.as_deref(),
        metadata.location_id.as_ref(),
        budget,
    )?)
}
...
pub(super) fn requirement_matches(
    ...
    match scope {
        ...
        ShiftScope::Filter {
            ...
            Ok(start_date_range
                .is_none_or(|range| range.start_date <= date && date < range.end_date_exclusive)
                && restriction(
                    assignment_type_ids.as_deref(),
                    Some(&metadata.assignment_type.id),
                    budget,
                )?
                && restriction(
                    location_ids.as_deref(),
                    metadata.location_id.as_ref(),
                    budget,
                )?)
        }
...
pub(super) fn qualified(
    ...
    match expression {
        ...
        QualificationExpression::Matches(expression) => {
            qualification_match(person, expression, query, budget)
        }
    }
}

pub(super) fn availability_matches(
    record: &Availability,
    metadata: &ShiftMetadata<'_>,
    budget: &mut OperationBudget<'_>,
) -> Result<bool, AssignmentRuleError> {
    budget.step()?;
    Ok(restriction(
        record.assignment_type_ids.as_deref(),
        Some(&metadata.assignment_type.id),
        budget,
    )? && restriction(
        record.location_ids.as_deref(),
        metadata.location_id.as_ref(),
        budget,
    )?)
}

pub(super) fn availability_failure(
    record: &Availability,
    shift: &ResolvedShift,
    ...
    let Some(query) = expanded.query else {
        return Ok(None);
    };
...
pub(super) fn pair_witness(
    input: &AssignmentInput,
    pair: crate::model::AssignmentPair,
    reason: &'static str,
) -> Result<Witness, AssignmentRuleError> {
    let metadata = input.metadata(input.shift(pair.shift_id).ok_or_else(invalid)?)?;
    Ok(Witness::pair(
        pair,
        metadata.assignment_type.id.as_entity_id(),
        "assignment_type",
        reason,
    ))
}
